...
export default function ProposalLinkField({ index, name, url }: LinkFieldProps) {
    ...
    return (
        <div className="flex w-full items-center space-x-5">
            <LinkNameInput placeholder="Name/Description" className="w-full" value={name} index={index} linksField={linksField} setLinksField={setLinksField} />
            <LinkUrlInput placeholder="Links" className="w-full" value={url} index={index} linksField={linksField} setLinksField={setLinksField} />
            <button onClick={() => handleRemoveLink(linksField, index, setLinksField)}><TrashIcon className="w-6 h-6 text-light-gray" /></button>
        </div>
    )
}