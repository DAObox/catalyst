    ...
    <CreateNewProposalAppShell className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex w-full space-x-5">
          <RadioInput
            placeholder="Now"
            className="w-full bg-gray-border p-4"
            customFunction={() => {
          ...
          <div className="relative w-full">
            <CalendarDaysIcon
              className="absolute right-3.5 top-4 z-10 h-6 w-6 text-white hover:cursor-pointer"
              onClick={() => startRef.current?.setOpen(true)}
            />
            <DatePicker
              onChange={(date) => handleDateChange(date?.toDateString() || "", setStartDate)}
              className="w-full rounded-md bg-gray-border p-4 pr-10 text-light-black outline-none placeholder:text-white"
      ...
      <div className="space-y-3">
        ...
        <div className="flex w-full space-x-5">
          ...
          <div className="relative w-full">
            <CalendarDaysIcon
              className="absolute right-3.5 top-4 z-10 h-6 w-6 text-white"
              onClick={() => endRef.current?.setOpen(true)}
            />
            <DatePicker
              onChange={(date) => handleDateChange(date?.toDateString() || "", setEndDate)}
              className="w-full rounded-md bg-gray-border p-4 pr-10 text-light-black outline-none placeholder:text-white"
              placeholderText="Custom"
              value={endDate}
            />
